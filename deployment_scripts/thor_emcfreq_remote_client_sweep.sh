...
OUT_DIR="${OUT_DIR:-thor_measurements/emc_sweep_vla_auto}"
...
EMC_FREQS="${EMC_FREQS:-665.6MHz 1.2GHz 1.6GHz 2.133GHz 2.75GHz 3.2GHz 3.75GHz 4.266GHz}"
...
if [[ -n "${FIXED_GPU_HZ}" ]]; then
  gpu_args+=(--fixed-gpu-hz "${FIXED_GPU_HZ}")
fi
...
python deployment_scripts/thor_emcfreq_power_sweep.py \
  --out-dir "${OUT_DIR}" \
